...
function getYearlyEarnedTimeValues(
  ...
  if ((current_year - hire_year) > 10) {
      ...
      if ((current_year > hire_year) && (current_month >= hire_month) && (current_day >= hire_day)) {
...
export function getSummaryValues(etInfo) {
  ...
  const values_arr              = getYearlyEarnedTimeValues(hire_date_month, 
                                  hire_date_day, hire_date_year);
  ...
  const total_et_before_hire    = ((yearly_et_before * 8) / total_yearly_paychecks) * total_paychecks_before;
  const total_et_after_hire     = ((yearly_et_after * 8) / total_yearly_paychecks) * total_paychecks_after;
  const total_et_for_year       = Math.round((total_et_before_hire + total_et_after_hire) * 100.0) / 100.0;
  const total_et                = Math.round((carry_over_et + total_et_for_year) * 100.0) / 100.0;        
  const et_to_burn              = Math.round((total_et - total_et_allowed - used_et) * 100.0) / 100.0;
  ...
  if (getSpecificDateValue(DateType.MONTH) <= hire_date_month &&
          getSpecificDateValue(DateType.DAY_OF_MONTH) < hire_date_day) {
      current_et_rate = Math.round(((yearly_et_before * 8) / total_yearly_paychecks) * 100.0) / 100.0;
  }
  // Else we're on or after my hire date (5/21)
  else {
      current_et_rate = Math.round(((yearly_et_after * 8) / total_yearly_paychecks) * 100.0) / 100.0;
  }
